...
class TikTokEngine extends BaseEngine {
    async discover(profileUrl, identifier) {
        const results = [];
        this.logDiagnostic(`   🚀 TikTok: Iniciando Discovery via Metadata Sniper (yt-dlp)...`);

        try {
            const command = `"${this.parent.binPath}" --flat-playlist --print-json --playlist-items 1-10 ${profileUrl}`;
            ...
            this.logDiagnostic(`   ❌ TikTok ERRO no Discovery: ${e.message}`);
        }
        
        this.logDiagnostic(`   ✨ TikTok: ${results.length} posts qualificados via Metadata Sniper.`);
        return results;
    }
}
